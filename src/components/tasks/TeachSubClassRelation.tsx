...
const TeachSubjectGradeClassForm: React.FC<Props> = ({
  ...
  const [selectAllSections, setSelectAllSections] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  const handleGradeSelection = (gradeId: string) => {
    setSelectedGrades((prev) => {
      const updated = prev.includes(gradeId)
  ...
  const toggleSelectAllSections = (gradeId: string) => {
    ...
    setSelectedSections((prev) => ({
      ...prev,
      [gradeId]: !selectAllSections[gradeId]
        ? sortedClasses.map((cls) => cls.id)
        : [],
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    console.log(selectedTeacher, selectedSubject, selectedSections, selectedGrades, selectedYear);
                    ...
                    <div key={gradeId} className="mb-5">
                      ...
                      <div className="ml-6 grid grid-cols-4 gap-4 mt-4">
                        {sortedClasses.map((cls) => (
                          ...
                          >
                            <Checkbox
                              checked={selectedSections[gradeId]?.includes(
                                cls.id
                              )}
                              onCheckedChange={() =>
                                handleSectionSelection(gradeId, cls.id)
                              }
                            />
