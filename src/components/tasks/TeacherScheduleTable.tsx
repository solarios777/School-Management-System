// components/TeacherScheduleTable.tsx
"use client";

import { useEffect, useState } from "react";
import { Table } from "@/components/ui/table";
import { Card } from "@/components/ui/card";
import { fetchPeriodTimetable, fetchTeacherSchedule } from "@/app/_services/scheduleRelated";
...
// Helper function to generate a unique color key for each grade-section combination
const getColorKey = (grade: number, className: string): string => {
  return `${grade}-${className}`;
};

export default function TeacherScheduleTable({ teacherId }: TeacherScheduleTableProps) {
  ...
  return (
    <Card className="p-4 shadow-lg">
      <h2 className="text-xl font-bold mb-4">Teacher&apos;s Schedule</h2>
            ...
            {rows.map((row) => (
              ...
              >
                ...
                {days.map((day) => {
                  ...
                  const gradeClass = `${cell.grade} ${cell.className}`;

                  // Generate a unique color key for the grade-section combination
                  const colorKey = getColorKey(cell.grade, cell.className);
                  const colorIndex =
                    Math.abs(colorKey.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0)) %
                  ...
                  return (
                    ...
                    >
                      <div className="p-2 border rounded-lg">
                        <div>{subject}</div>
                        <strong className="text-sm text-bold text-black">{gradeClass}</strong>
                      </div>
                    </td>
                  );
                })}
